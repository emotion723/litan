...
export const StarShift1_Black = () => {
  ...
  return (
    <svg className="arena-svg" viewBox="0 0 1920 1920" style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      <rect x={OFFSET} y={OFFSET} width={SQUARE_SIZE} height={SQUARE_SIZE} fill="none" stroke="#ff2d55" strokeWidth="10" opacity="0.25" />
      {time < 3 ? (
        <text x={CENTER} y="150" textAnchor="middle" fill="#ffd700" fontSize="100" fontWeight="900">星移倒數: {Math.max(0, 3 - time).toFixed(1)}s</text>
      ) : time < 12 ? (
      ...
      <circle cx={CENTER} cy={CENTER} r={BOSS_RADIUS} fill="#ff2d55" stroke="white" strokeWidth="5" />
    </svg>
  );
};